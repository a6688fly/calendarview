...
public class CalendarLayout extends LinearLayout {
    ...
    //   设置是在我的和团队任务状态，当显示的是我的任务状态的时候，选择团队任务布局就不显示出来
...
//    private static final int GESTURE_MODE_ONLY_CALENDAR = 1;
    ...
    public CalendarLayout(Context context, AttributeSet attrs) {
        ...
        mTopViewId = array.getResourceId(R.styleable.CalendarLayout_calendar_top_view_id, 0);
        mDefaultStatus = array.getInt(R.styleable.CalendarLayout_default_status, STATUS_SHRINK);
     ...
     * @param cur 当前日期时间
     */
    public void initCalendarPosition(Calendar cur) {
...
//
                if (dy < 0 && mContentView.getTranslationY() == -mContentViewTranslateY) {
                    if (null !=mTopView &&mTopView.getVisibility() ==VISIBLE){
                        mTopView.setVisibility(GONE);
                    }
                    return false;
                }

//                Log.e("-----------",dy+":::"+mContentView.getTranslationY()+":::"+-mContentViewTranslateY+":::"+markY);

                if(dy>=0 && mContentView.getTranslationY() == -mContentViewTranslateY && markY>600){
                    return false;
                }
                /*
                 * 如果向下滚动，有 2 种情况处理 且y在ViewPager下方
                ...
                if (  dy > 20 && mContentView.getTranslationY() == 0 && y >= CalendarUtil.dipToPx(getContext(), 98)) {
//                    Log.e("----------------","MotionEvent.ACTION_MOVE3"+isTeam);
                    if (isTeam) {
                        if (null != mTopView) {
                            mTopView.setVisibility(VISIBLE);
                            if (null != topViewVisibleListener) {
                                topViewVisibleListener.isVisible();
                ...
                if (Math.abs(dy) > mTouchSlop) {//大于mTouchSlop开始拦截事件，ContentView和ViewPager得到CANCEL事件
                    Log.v("TAG","MotionEvent.ACTION_MOVE4");

                    if ((dy > 20 && mContentView.getTranslationY() <= 0)
                            || (dy < -20 && mContentView.getTranslationY() >= -mContentViewTranslateY)) {
                        Log.v("TAG","MotionEvent.ACTION_MOVE5");
                        mLastY = y;
                        return true;
                    }
                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mContentView != null && mMonthView != null) {
            ...
            int h = height - mItemHeight
                    ...
                    - CalendarUtil.dipToPx(getContext(), 1)-CalendarUtil.dipToPx(getContext(), 50);
    ...
    protected void onFinishInflate() {
        ...
        mTopView = (RelativeLayout) findViewById(mTopViewId);
        if (getChildCount() > 0) {
     ...
     * 平移ViewPager月视图
     */
    public void translationViewPager() {
    ...
    public TopViewVisibleListener topViewVisibleListener;
    public void setTopViewVisibleListener(TopViewVisibleListener topViewVisibleListener){

        this.topViewVisibleListener = topViewVisibleListener;
    }
    public interface  TopViewVisibleListener{

        void isVisible();
    }


    public boolean isTeam(boolean isTeam){
        this.isTeam = isTeam;
        return  this.isTeam;
    }
}
